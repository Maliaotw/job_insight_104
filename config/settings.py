...
# 數據庫設置
DATABASE_PROCESSED_DATA_PATH = BASE_DIR / os.environ.get("DATABASE_PROCESSED_DATA_PATH", "data/processed_job_data.duckdb")

# 爬蟲設置
CRAWLER_KEYWORDS_STR = os.environ.get("CRAWLER_KEYWORDS", "flask,Python,DevOps,SRE,fastapi,django")
CRAWLER_KEYWORDS = [keyword.strip() for keyword in CRAWLER_KEYWORDS_STR.split("|")]
CRAWLER_SCHEDULE_DAILY_CRAWL_HOUR = int(os.environ.get("CRAWLER_SCHEDULE_DAILY_CRAWL_HOUR", "10"))
CRAWLER_SCHEDULE_DAILY_CRAWL_MINUTE = int(os.environ.get("CRAWLER_SCHEDULE_DAILY_CRAWL_MINUTE", "0"))
...
TAIWAN_CITY = {
    ...
    "新北市": [
        ...
        "蘆洲區", "樹林區", "鶯歌區", "三峽區", "淡水區", "汐止區", "瑞芳區",
        "五股區", "泰山區", "林口區", "八里區", "三芝區", "石門區", "金山區",
        "萬里區", "平溪區", "雙溪區", "貢寮區", "深坑區", "石碇區", "坪林區",
        "烏來區"
    ],
    "桃園市": [
        "桃園區", "中壢區", "平鎮區", "八德區", "楊梅區", "蘆竹區", "大溪區",
        "大園區", "龜山區", "龍潭區", "新屋區", "觀音區", "復興區"
    ],
    "新竹市": ["東區", "北區", "香山區"],
    "新竹縣": [
