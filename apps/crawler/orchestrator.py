...
class CrawlerOrchestrator:
    ...
    def __init__(self, storage: Optional[JobStorage] = None, output_dir: Optional[str] = None):
        """
        初始化爬蟲協調器
    ...
    async def crawl(self, keywords: List[str]) -> List[Dict]:
        ...
        try:
            ...
            # 步驟2: 爬取所有頁面的職缺數據
            search_results = await self.searcher.fetch_all_pages(keyword_to_urls_mapping)
            ...
            for keyword, urls in keyword_to_urls_mapping.items():
                ...
                keyword_results = [
                    ...
                    if result.get('keyword') == keyword or any(url in result.get('url', '') for url in urls)
                ]
