...
class JobSearcher:
    ...
    def build_url(self, keyword: str = "", page: int = 1, area: str = "", 
                 industry: str = "", job_category: str = "", 
                 experience: str = "", education: str = "") -> str:
        """
    ...
    async def search_keyword(self, keyword: str, area_code_map: Dict[str, str]) -> List[str]:
        ...
        # 分離成功和失敗的結果
        successful_results = [result for result in first_page_results if isinstance(result, dict) and result.get('status') == 'ok']
        failed_results = [result for result in first_page_results if not isinstance(result, dict) or result.get('status') != 'ok']
        ...
        all_page_urls = list(itertools.chain.from_iterable([result['result'] for result in successful_results if result and 'result' in result]))
    ...
    async def fetch_all_pages(self, keyword_to_urls_mapping: Dict[str, List[str]]) -> List[Dict]:
        """
        爬取所有頁面的職缺數據
        ...
        # 分離成功和失敗的結果
        successful_results = [result for result in search_results if isinstance(result, dict) and result.get('status') == 'ok']
        failed_count = sum(1 for result in search_results if not isinstance(result, dict) or result.get('status') != 'ok')
        
        logger.info(f"成功: {len(successful_results)}個, 失敗: {failed_count}個")
        
        return successful_results
    
    def extract_jobs_from_results(self, search_results: List[Dict], keyword: str = "") -> List[Dict]:
        """
        從搜索結果中提取職缺數據
