...
def prepare_jobs_analysis_df(jobs_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 添加基本職缺信息
    jobs_analysis['職缺編號'] = jobs_df['jobNo'] if 'jobNo' in jobs_df.columns else range(len(jobs_df))
    jobs_analysis['職稱'] = jobs_df['jobName'] if 'jobName' in jobs_df.columns else "未知"
    jobs_analysis['公司名稱'] = jobs_df['custName'] if 'custName' in jobs_df.columns else "未知"
    ...
    if 'appearDate' in jobs_df.columns:
        jobs_analysis['上架日期'] = pd.to_datetime(jobs_df['appearDate'], format='%Y%m%d')
        jobs_analysis['在架天數'] = (pd.Timestamp.now() - jobs_analysis['上架日期']).dt.days
...
def extract_application_counts(jobs_analysis: pd.DataFrame) -> pd.DataFrame:
    ...
    # 計算平均應徵人數
    result_df['平均應徵人數'] = (result_df['最少應徵人數'] + result_df['最多應徵人數']) / 2
...
def get_job_display_columns(df: pd.DataFrame = None) -> List[str]:
    ...
    """
    # 定義標準顯示欄位
    standard_cols = ['城市', '地區', '職稱', '公司名稱', '在架天數', '上架日期', '連結', 
                     '薪資範圍', '應徵人數範圍', '工作經驗', '教育程度', '關鍵字']
