...
class TrendAnalyzer:
    ...
    def create_job_trend_chart(self, jobs_df):
        ...
        #
        logger.debug(f"獲取到 {len(stats_df) if not stats_df.empty else 0} 條每日統計數據")
        return stats_df

    def analyze_daily_job_changes(self, jobs_df: pd.DataFrame, date_column: str = 'appearDate', date_format: str = '%Y%m%d') -> pd.DataFrame:
        """
        分析每日職缺新增和減少情況。
        ...
        # 將日期欄位轉換為日期格式
        jobs_df['appear_date'] = pd.to_datetime(jobs_df[date_column], format=date_format)
        ...
        if 'delisted_date' in jobs_df.columns:
            ...
            jobs_df['delisted_date'] = pd.to_datetime(jobs_df['delisted_date'], errors='coerce')
            max_delisted = jobs_df['delisted_date'].max()
            max_date = max(jobs_df['appear_date'].max(), max_delisted) if not pd.isna(max_delisted) else jobs_df['appear_date'].max()
        else:
        ...
        new_jobs_by_date = jobs_df.groupby('appear_date').size().reset_index(name='new_jobs')
        ...
        if 'delisted_date' in jobs_df.columns:
            ...
            if not valid_delisted.empty:
                removed_jobs_by_date = valid_delisted.groupby('delisted_date').size().reset_index(name='removed_jobs')
                daily_jobs = daily_jobs.merge(removed_jobs_by_date, left_on='appear_date', right_on='delisted_date', how='left')
        ...
        daily_jobs['jobNo'] = daily_jobs['new_jobs'].cumsum() - daily_jobs['removed_jobs'].cumsum()
    ...
    def analyze_job_details_by_date(self, jobs_df: pd.DataFrame, date_column: str = 'appearDate', date_format: str = '%Y%m%d') -> pd.DataFrame:
        """
    ...
    def analyze_industry_distribution(self, jobs_df: pd.DataFrame, industry_column: str = 'coIndustryDesc') -> pd.DataFrame:
        """
        分析產業職缺分佈統計。
        ...
        # 計算產業分佈統計
        industry_dist = jobs_df.groupby(industry_column).agg({
            'jobNo': 'count',
            'custName': 'nunique',
            'jobName': list
        }).reset_index()
    ...
    def analyze_industry_trends(self, jobs_df: pd.DataFrame, date_column: str = 'appearDate',
                               industry_column: str = 'coIndustryDesc', date_format: str = '%Y%m%d') -> pd.DataFrame:
        """
        按產業和月份分析職缺趨勢。
        ...
        # 將日期轉換為月份格式
        jobs_df['month'] = pd.to_datetime(jobs_df[date_column], format=date_format).dt.strftime('%Y-%m')
        ...
        monthly_industry_stats = monthly_industry_stats.rename(columns={industry_column: '產業類別'})

        # 計算增長量和增長率
        month_cols = [col for col in monthly_industry_stats.columns if col != '產業類別']
        if len(month_cols) >= 2:
            monthly_industry_stats['增長量'] = monthly_industry_stats[month_cols].iloc[:, -1] - monthly_industry_stats[month_cols].iloc[:, 0]
            ...
            monthly_industry_stats['增長率'] = np.where(
                ...
                (monthly_industry_stats['增長量'] / monthly_industry_stats[month_cols].iloc[:, 0] * 100).round(2).astype(str) + '%'
            )

        logger.info(f"成功分析產業月度職缺趨勢，共{len(monthly_industry_stats)}個產業類別")
