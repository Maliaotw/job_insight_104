...
from apps.visualization.analysis.df_utils import prepare_jobs_analysis_df, get_job_display_columns
...
def display_jobs_for_selected_date(selected_date, jobs_df, trend_analyzer=None):
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "職稱": st.column_config.TextColumn(
                "職稱",
                width="large"
            ),
            ...
            "薪資範圍": st.column_config.TextColumn(
                "薪資範圍",
                width="medium"
            ),
            ...
            "地區": st.column_config.TextColumn(
                "地區",
                width="small"
            )
        },
...
class JobDataLoader:
    ...
    def load_jobs(self, keywords: Optional[List[str]] = None, 
                 city: Optional[str] = None, 
                 district: Optional[str] = None, 
                 limit: int = DEFAULT_JOB_LIMIT, 
                 months: Optional[int] = None) -> Optional[pd.DataFrame]:
        """
        從數據庫載入職缺數據。
    ...
    def filter_by_location(self, jobs_df: pd.DataFrame, 
                          selected_city: str, 
                          selected_district: str) -> Optional[pd.DataFrame]:
        """
        根據選擇的城市和地區過濾職缺數據。
        ...
        """
        logger.info(f"根據選擇的城市和地區過濾職缺: 城市={selected_city}, 地區={selected_district}")
        ...
        if filtered_df.empty:
            filter_desc = self._get_location_filter_description(selected_city, selected_district)
    ...
    def _get_location_filter_description(self, selected_city: str, selected_district: str) -> str:
        """
        獲取位置過濾條件描述。
        ...
        返回:
            filter_desc: 過濾條件描述
        """
        if selected_city != ALL_CITIES_LABEL and selected_district != ALL_DISTRICTS_LABEL:
...
class DailyChangesAnalyzer:
    ...
    def analyze_daily_changes(self, jobs_df: pd.DataFrame) -> None:
        ...
        logger.debug(f"獲取到 {len(daily_jobs) if not daily_jobs.empty else 0} 條每日職缺變化數據")
        ...
        logger.debug(f"數據時間範圍: {daily_jobs['appear_date'].min() if not daily_jobs.empty else 'N/A'} 至 {daily_jobs['appear_date'].max() if not daily_jobs.empty else 'N/A'}")

        self._display_analysis_results(daily_jobs, jobs_df)

    def _display_analysis_results(self, daily_jobs: pd.DataFrame, jobs_df: pd.DataFrame) -> None:
        """
    ...
    def provide_date_selector_for_detailed_analysis(self, jobs_df, on_date_selected: Optional[Callable] = None):
        """
        提供日期選擇器進行詳細分析
        ...
        logger.debug(f"獲取到 {len(job_details) if not job_details.empty else 0} 條職缺詳細變化數據")
        ...
        logger.debug(f"詳細數據時間範圍: {job_details['appear_date'].min().strftime('%Y-%m-%d') if not job_details.empty else 'N/A'} 至 {job_details['appear_date'].max().strftime('%Y-%m-%d') if not job_details.empty else 'N/A'}")
        ...
        selected_date = st.selectbox("選擇日期查看詳細變化", dates, key="daily_changes_date")
    ...
    def analyze_selected_date_changes(self, job_details, selected_date):
        ...
        selected_idx = job_details[job_details['appear_date'] == selected_date_obj].index
        logger.debug(f"選定日期索引: {selected_idx[0] if len(selected_idx) > 0 else 'N/A'}")

        if len(selected_idx) == 0 or selected_idx[0] <= 0:
            logger.warning(f"無法比較 {selected_date} 與前一天的數據")
            st.info(f"無法比較 {selected_date} 與前一天的數據，可能是因為這是數據中的第一天。")
            return
        ...
        logger.debug(f"當前日期職缺數: {len(curr_row['jobName'])}, 前一日期職缺數: {len(prev_row['jobName'])}")
        ...
        # 顯示結果
        self.display_job_change_results(curr_date, curr_idx, job_details, new_job_keys, removed_job_keys, curr_df, prev_df)
    ...
    def calculate_job_changes(self, curr_df, prev_df):
        ...
        logger.debug(f"當前日期不重複職缺: {len(curr_jobs_keys)}, 前一日期不重複職缺: {len(prev_jobs_keys)}")

        # 計算新增和減少的職缺鍵
        new_job_keys = curr_jobs_keys - prev_jobs_keys
        removed_job_keys = prev_jobs_keys - curr_jobs_keys
        logger.debug(f"新增職缺數: {len(new_job_keys)}, 減少職缺數: {len(removed_job_keys)}")

        return new_job_keys, removed_job_keys

    def display_job_change_results(self, curr_date, curr_idx, job_details, new_job_keys, removed_job_keys, curr_df, prev_df):
        """
        顯示職缺變化結果
...
class DailyChangesPageRenderer:
    ...
    def render_page(self, keywords: Optional[List[str]] = None, 
                   city: Optional[str] = None, 
                   district: Optional[str] = None, 
                   limit: int = DEFAULT_JOB_LIMIT, 
                   months: Optional[int] = None) -> None:
        """
        渲染每日職缺變化分析頁面。
        ...
        try:
            # 載入數據
            jobs_df = self.job_data_loader.load_jobs(keywords, city, district, limit, months)
            if jobs_df is None:
                return
...
def display_monthly_calendar_view(daily_jobs, jobs_df, on_date_selected: Optional[Callable] = None):
    """
    ...
    with month_container:
        ...
        with col2:
            # 添加一個小的說明
            st.markdown("<div style='margin-top: 30px;'>選擇月份查看該月職缺變化</div>", unsafe_allow_html=True)
        ...
        for row in range(num_rows):
            ...
            for col in range(7):
                ...
                if day_idx < 0 or day_idx >= len(days_in_month):
                    ...
                    # 查找當天的新增職缺數
                    day_data = month_data[month_data['appear_date'].dt.date == current_date.date()]
                    new_jobs_count = day_data['new_jobs'].sum() if not day_data.empty else 0
                    ...
                    if new_jobs_count > 0:
                        # 計算顏色強度 - 根據最大值進行歸一化
                        color_intensity = min(0.2 + (new_jobs_count / max(max_new_jobs, 1)) * 0.8, 1.0)
                        ...
                        # 普通顯示沒有新增職缺的日期
                        # 根據是否為今天和週末設置不同的樣式
                        bg_color = "#f0f2f6"  # 默認背景色 - 使用與其他元素相同的淺藍色背景
                        text_color = "#333333"  # 默認文字顏色
                        border = ""
...
def display_daily_job_trend_chart(daily_jobs):
    ...
    logger.debug(f"圖表數據範圍: 新增職缺 {daily_jobs['new_jobs'].min()} 至 {daily_jobs['new_jobs'].max()}, 減少職缺 {daily_jobs['removed_jobs'].min()} 至 {daily_jobs['removed_jobs'].max()}")

    # 計算淨變化
    daily_jobs['淨變化'] = daily_jobs['new_jobs'] - daily_jobs['removed_jobs']
    chart_data = daily_jobs.rename(columns={
        'appear_date': 'date',
        'new_jobs': '新增職缺',
        'removed_jobs': '減少職缺'
    })
...
def display_daily_job_details_table(daily_jobs):
    ...
    logger.debug(f"表格數據準備完成，包含 {len(display_df)} 行，{len(display_df.columns)} 列")
...
def show_daily_changes_page(job_data_analyzer, trend_analyzer, keywords=None, city=None, district=None, limit=DEFAULT_JOB_LIMIT, months=None):
    """
